...
/** create_element([parent,] [tag_name,], [attrs [,options]])
 *  @param {Node} parent (optional)
 *  @param {string} tag_name (optional, default "div")
 *  @param {Object|undefined|null} attrs
 *  @param {Object|undefined|null} options (optional)
 *  @return {Element} the new element
 *  If attrs contains an "id" entry, an element with that id must not already
 *  exist, otherwise an error will be thrown.
 *  parent and tag_name may be specified either as arguments or in the options
 *  object.  For these, if the value is specified in both places and the
 *  specified values do not match, then an error will be thrown.
 */
export function create_element(...args) {
    let parent, tag_name = 'div', attrs, options;
    ...
    // parse argument list
    {
        ...
        options = args[idx++];
        if ( (args.length > idx) ||
             (attrs   !== null && typeof attrs   !== 'undefined' && typeof attrs   !== 'object') ||
             (options !== null && typeof options !== 'undefined' && typeof options !== 'object')    ) {
    ...
    const {
        ...
        tag_name: tag_name_option,
        prepend,
    } = (options ?? {});

    // validate arguments
    if (parent_option) {
        if (parent && (parent !== parent_option)) {
            throw new Error('parent specified in arguments and in options but with different values');
        }
        parent = parent_option;
    }
    if (parent && !(parent instanceof Node)) {
        throw new Error('parent must be an instance of Node');
    }
    if (tag_name_option) {
        if (tag_name && (tag_name !== tag_name_option)) {
            throw new Error('tag_name specified in arguments and in options but with different values');
        }
        tag_name = tag_name_option;
    }
    ...
    // add the element to the given parent, if any
    if (parent) {
        if (prepend) {
...
export function create_stylesheet_link(parent, stylesheet_url, attrs, permit_duplication=false) {
    ...
    return link_element ?? create_element(parent, 'link', {
        rel: "stylesheet",
        href: stylesheet_url,
        ...attrs,
    });
...
export function create_script(parent, script_url, attrs, permit_duplication=false) {
    ...
    return script_element ?? create_element(parent, 'script', {
        src: script_url,
        ...attrs,
    });
}
