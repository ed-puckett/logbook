...
const {
    OpenPromise,
} = await import('../../sys/open-promise.js');


// === STYLESHEET AND POLYFILL ===

create_stylesheet_link(document.head, new URL('./dialog.css', import.meta.url));
...
export class Dialog {
    ...
    _create_dialog_element() {
        ...
        const content_element = document.getElementById('content') ??
              create_element(document.body, { id: 'content' });
        ...
        const ui_element = document.getElementById('ui') ??
              create_element(content_element, { id: 'ui' }, { prepend: true });
        ...
        const dialog_element = create_element(ui_element, 'dialog', {
            id: this._dialog_element_id,
            class: this.constructor._modal_dialog_css_class,
        });
        this._dialog_text_container = create_element(dialog_element, {
            class: 'dialog_text',
        });
        this._dialog_form = create_element(dialog_element, 'form', {
            method: 'dialog',
        });
...
export class ConfirmDialog extends Dialog {
    _populate_dialog_element(message, options) {
        ...
        this._dialog_text_container.innerText = message;
        const decline_button = create_element(this._dialog_form, 'input', {
            type: 'button',
