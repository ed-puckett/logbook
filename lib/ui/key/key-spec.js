export class KeySpec {
    ...
    static from_keyboard_event(keyboard_event) {
        ...
        return new KeySpec(key_string, keyboard_event);
    }

    get is_printable (){
        if (!this.#is_printable) {
            // cache result
            if ((this.modifier_flags & ~this.constructor.shift_flag) !== 0) {
                // not printable if any modifier other than shift is applied
                this.#is_printable = false;
            } else {
                this.#is_printable = !this.key.match(/^[\p{C}]$/u);//!!! check this
            }
        }
        return this.#is_printable;
    }
    #is_printable;  // memoization
    ...
    #init() {
        ...
        const key = modifiers[modifiers.length-1];  // note: not converted to lowercase
        ...
        const canonical_modifiers = modifier_descs
              .map(desc => desc.code)
              .join('');
        // note: use lower-case key if there are modifiers, otherwise preserve alphabetic case
        const canonical = `${canonical_modifiers}${this.constructor.canonical_key_modifier_separator}${canonical_modifiers ? klc : key}`;
