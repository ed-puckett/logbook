...
export class Evaluator extends StoppableObjectsManager {
     ...
     */
    static async class_for_content(input_element) {
        if (!(input_element instanceof Element)) {
            throw new Error('input_element must be an instance of Element');
        }

        const evaluator_classes = await get_evaluator_classes();
 ...
 */
export async function get_evaluator_classes() {
    // do these imports here to avoid import cycles
    const { MarkdownEvaluator   } = await import(new URL('./markdown-evaluator.js',   current_script_url));
    const { TeXEvaluator        } = await import(new URL('./tex-evaluator.js',        current_script_url));
    const { JavaScriptEvaluator } = await import(new URL('./javascript-evaluator.js', current_script_url));
    return [
