...
const {
    generate_object_id,
} = await import('../../lib/sys/uuid.js');

const {
    sha256,
} = await import('../../lib/sys/sha.js');

const {
    fs_interface,
} = await import('../../lib/sys/fs-interface.js');
...
const {
    SettingsDialog,
} = await import('../settings-dialog/_.js');
...
const {
    get_theme_settings,
    theme_settings_updated_events,
} = await import('../theme-settings/_.js');

const {
    KeyMap,
} = await import('../../lib/ui/key/key-map.js');

const {
    menu_command_events,
    MenuBar,
} = await import('../../lib/ui/menu/_.js');

const {
    open_help_window,
} = await import('../help-window.js');
...
const {
    ...
    clean_for_html,
    output_handlers,
} = await import('../output-handlers/_.js');

const {
    create_output_context,
} = await import('../output-context.js');
...
const {
    ...
    EvalAgent,
} = await import('../eval-agent.js');

const {
    EvalWorker,
} = await import('../eval-worker/_.js');

const {
    initializing_data_element_id,
    create_exported_logbook,
} = await import('../create-exported-logbook.mjs');
...
let settings        = get_settings();        // updated by settings_updated_events events
let theme_settings  = get_theme_settings();  // updated by theme_settings_updated_events events

settings_updated_events.subscribe((event) => {
    settings = get_settings();
    logbook?.update_from_settings();
});

theme_settings_updated_events.subscribe(() => {
...
export const logbook_ready = new Promise((resolve, reject) => {
    try {
        ...
        // (used in ./output-handlers/_.js) still requires the older version.
...
class Logbook {
    ...
    async _initialize_document() {
        ...
        const content_el = create_element(document.body, { id: 'content' });

        this.controls         = create_element(content_el, { id: 'controls' });
        this.interaction_area = create_element(content_el, { id: 'interaction_area' });

        const indicators_el = create_element(this.controls, { id: 'indicators' });
        this.modified_indicator   = create_element(indicators_el, { id: 'modified_indicator',   title: 'Modified' });
        this.running_indicator    = create_element(indicators_el, { id: 'running_indicator',    title: 'Running' });
        this.formatting_indicator = create_element(indicators_el, { id: 'formatting_indicator', title: 'Formatting' });

        // add logbook stylesheet:
        const stylesheet_url = new URL('main.css', import.meta.url);
        create_stylesheet_link(document.head, stylesheet_url);

        // add menu stylesheet:
        const menu_stylesheet_url = new URL('../../lib/ui/menu/menu.css', import.meta.url);
        ...
        await load_cm_script('../codemirror-mdmj-mode.js');
    ...
    init_event_handlers() {
        //!!! these subscribes are never unsubscribed
//!!! UPDATE:       KeyBindingCommandEvent.subscribe(async (event) => this.handle_command(event.command, event.context));
        menu_command_events.subscribe(async (event) => this.handle_command(event.command));
    ...
    async handle_command(command, context=undefined) {
